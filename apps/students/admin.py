...
class StudentAdmin(admin.ModelAdmin):
    ...
    ordering = ('-updated_at',)
    readonly_fields = ('created_at', 'updated_at', 'created_by', 'updated_by', 'last_changed_field')

    fieldsets = (
        ('Личная информация', {'fields': ('first_name', 'last_name', 'patronymic', 'age')}),
        ...
        ('Метаданные', {'fields': ('created_at', 'created_by', 'updated_at', 'updated_by', 'last_changed_field')}),
    )
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    required = ['Фамилия', 'Имя', 'Возраст', 'Личный телефон']
                    ...
                    for idx, row in df.iterrows():
                        try:
                            data = {
                                ...
                                'age': int(row['Возраст']) if pd.notna(row.get('Возраст')) else 0,
                            ...
                            if data['age'] < 14 or data['age'] > 30:
                                raise ValueError("Возраст от 14 до 30 лет")
                    ...
                    if errors:
                        messages.warning(request, f"Создано {created} студентов. Ошибки в {len(errors)} строках.")
                        for error in errors[:10]:
                            messages.warning(request, error)
                    else:
                        messages.success(request, f"Успешно создано {created} студентов!")
    ...
    # Автозаполнение кем создан/изменён
    ...
    updated_by_display.admin_order_field = 'updated_by'
...
class LevelHistoryAdmin(admin.ModelAdmin):
    list_display = ('student', 'old_level', 'new_level', 'changed_by', 'changed_at', 'comment')
    list_filter = ('old_level', 'new_level', 'changed_by')
    search_fields = ('student__first_name', 'student__last_name', 'changed_by__username')
    ordering = ('-changed_at',)
